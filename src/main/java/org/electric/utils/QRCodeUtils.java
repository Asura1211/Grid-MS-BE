...
public class QRCodeUtils {
    ...
    private static BufferedImage createImage(String content, String imgPath, boolean needCompress ,int size) throws Exception {
        ...
        // 插入图片
        QRCodeUtils.insertImage(image, imgPath, needCompress,size);
        return image;
    }
    ...
    public static BufferedImage encode(String content) {
        ...
        try {
            image = QRCodeUtils.createImage(content, null, false,80);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return image;
    ...
    public static String encode(String content, String imgPath, String destPath, boolean needCompress) throws Exception {
        Integer fileName = new Random().nextInt(99999999);
        return encode(content, imgPath, destPath, needCompress,String.valueOf(fileName));
    }
     ...
     * @throws Exception
     */
    public static String encode(String content, String imgPath, String destPath, boolean needCompress, String name) throws Exception {
        BufferedImage image = QRCodeUtils.createImage(content, imgPath, needCompress,300);
        mkdirs(driveLetter+":\\" + uploadPath + "\\" + destPath);
        String file = name+".jpg";
        ImageIO.write(image, FORMAT_NAME, new File(driveLetter+":\\" + uploadPath + "\\" + destPath + "\\" +file));
        return file;
    }
